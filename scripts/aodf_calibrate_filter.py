...
def _filter_sh_functional(in_sh, sh_order, sh_basis, full_basis,
                          ...
                          sf_range, j_invariance=False, channel_axis=-1):
...
def grid_search(in_sh, denoise_func, aggregate_func, tested_parameters, crop_size):
    ...
    noisy_input = aggregate_func(in_sh, full_basis=in_full)
    losses = []
    parameters = []
    min_loss = np.inf
    ...
    for args in _product_from_dict(tested_parameters):
        ...
        denoised = aggregate_func(out_sh, full_basis=True)
...
def main():
    ...
    if ext != '.csv':
        parser.error('Invalid extension for output.')
